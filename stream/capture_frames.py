...
def parse_args():
    ...
    p.add_argument(
        "--total", type=int, default=200, help="Total de frames a capturar"
    )
    p.add_argument(
        "--interval",
        type=float,
    ...
    def read(self):
        while True:
            start = self._buf.find(b"\xff\xd8")
                ...
                self._buf.find(b"\xff\xd9", start + 2) if start != -1 else -1
            )
...
def main():
    ...
    try:
        while saved < args.total:
            if args.manual:
                input(
                    f"  [{saved:>3}/{args.total}] Pressione ENTER para capturar..."
                )
                if args.source == "mjpeg":
