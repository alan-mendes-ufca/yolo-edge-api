...
def _read_next_frame(proc, leftover: bytes):
    """
    Lê o próximo frame JPEG completo disponível no stdout do rpicam-vid.
...
def parse_args():
    p = argparse.ArgumentParser(description="Diagnóstico ingênuo de pipeline de vídeo")
...
def main():
    ...
    print(f"  Captura média     : {total_capture / max(n, 1):>7.1f} ms")
    print(f"  Inferência média  : {total_infer / max(n, 1):>7.1f} ms")
